...
def computeDelta(wt, X, Xi):
	...
	def similarity(a,b):
		...
		similarity_num = sum((a - a_mean)*(b - b_mean))
		similarity_den = len(a) * a_std * b_std
		return (similarity_num/similarity_den)
	
	numerator = 0
	denominator = 0
	x_val = X.values[:-1]
	for x_i in Xi.iterrows():
		x_i_val = x_i[1].tolist()[:-1]
		e_similarity = math.exp(wt * similarity(x_val, x_i_val))
		numerator += x_i[1].Yi * e_similarity
		denominator += e_similarity
